...
class MyCompose:
    ...
    def __init__(self, project_name, project_dir='.', file_compose='docker-compose.yml'):
        ...
        options = {'--host': '/var/run/docker.sock', '--compatibility':True     }
        print("options of compose: {}".format(options))
        self.compose = TopLevelCommand(self._project, project_dir=project_dir, options=options)
    ...
    def build(self, services=None):
        ...
        services_no_scanner = list(filter(lambda x: x != "scanner" or x != "dashboard", services))
        print(se)
